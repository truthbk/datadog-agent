...
namespace Datadog.CustomActions.Native
{
    ...
    public static class NativeMethods
    {
        ...
        {
            var sid = new byte[userSid.BinaryLength];
            userSid.GetBinaryForm(sid, 0);
            uint sz = 0;
            if (!GetWindowsAccountDomainSid(sid, null, ref sz) && sz == 0)
            {
                // This will fail if the SID is not a domain SID (i.e. a local account)
                return false;
            }
            sid = new byte[sz];
            if (GetWindowsAccountDomainSid(sid, sid, ref sz))
            {
                var domainSid = new SecurityIdentifier(sid, 0);

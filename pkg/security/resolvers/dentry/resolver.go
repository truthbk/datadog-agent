...
type Resolver struct {
	...
	cache                 map[uint32]*lru.Cache[uint64, *DentryCacheEntry]
	erpc                  *erpc.ERPC
	erpcSegment           []byte
	erpcSegmentSize       int
...
func (dr *Resolver) lookupInodeFromCache(mountID uint32, inode uint64) (*DentryCacheEntry, error) {
...
func (dr *Resolver) cacheInode(key model.DentryKey, dentry *DentryCacheEntry) error {
	...
	dentry.Generation = dr.cacheGeneration.Load()
...
func (dr *Resolver) lookupInodeFromMap(mountID uint32, inode uint64, pathID uint32) (model.DentryLeaf, error) {
	key := model.DentryKey{MountID: mountID, Inode: inode, PathID: pathID}
	var pathLeaf model.DentryLeaf
	if err := dr.dentries.Lookup(key, &pathLeaf); err != nil {
		return pathLeaf, fmt.Errorf("unable to get filename for mountID `%d` and inode `%d`: %w", mountID, inode, err)
	}
	return pathLeaf, nil
}

func (dr *Resolver) getPathEntryFromPool(parent model.DentryKey) *DentryCacheEntry {
...
// 	for i := 0; i <= model.MaxPathDepth; i++ {
// 		key.Write(keyBuffer)
// 		if err := dr.dentries.Lookup(keyBuffer, &pathLeaf); err != nil {
// 			filenameParts = nil
...
func (dr *Resolver) GetParent(mountID uint32, inode uint64, pathID uint32) (uint32, uint64, error) {
	...
	if err != nil && dr.config.ERPCDentryResolutionEnabled {
		...
		if err == nil && parentMountID != 0 && parentInode != 0 && !IsFakeInode(inode) {
			...
			entry := dr.getPathEntryFromPool(parentKey)

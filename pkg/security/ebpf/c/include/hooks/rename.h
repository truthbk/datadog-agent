...
int kprobe_vfs_rename(struct pt_regs *ctx) {
    ...
    if (inode) {
        invalidate_inode(ctx, syscall->rename.target_file.dentry_key.mount_id, inode, 1);
    }
...
int __attribute__((always_inline)) sys_rename_ret(void *ctx, int retval, int dr_type) {
    ...
    if (retval >= 0) {
        invalidate_inode(ctx, syscall->rename.target_file.dentry_key.mount_id, syscall->rename.target_file.dentry_key.ino, !pass_to_userspace);

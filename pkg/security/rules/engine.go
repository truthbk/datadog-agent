...
func NewRuleEngine(evm *eventmonitor.EventMonitor, config *config.RuntimeSecurityConfig, probe *probe.Probe, rateLimiter *events.RateLimiter, apiServer APIServer, sender events.EventSender, statsdClient statsd.ClientInterface, selfTester *selftests.SelfTester) *RuleEngine {
	return &RuleEngine{
		probe:                     probe,
		config:                    config,
		apiServer:                 apiServer,
...
func (e *RuleEngine) Start(ctx context.Context, wg *sync.WaitGroup) error {
	if e.config.SelfTestEnabled && e.selfTester != nil {
		if triggerred, err := e.RunSelfTest(true); err != nil {
...
func (e *RuleEngine) RunSelfTest(sendLoadedReport bool) (bool, error) {
	prevProviders, providers := e.policyProviders, e.policyProviders
	if len(prevProviders) > 0 {
		defer func() {
			if err := e.LoadPolicies(prevProviders, false); err != nil {
				seclog.Errorf("failed to load policies: %s", err)
			}
		}()
	}
	...
	providers = append(providers, e.selfTester)

	if err := e.LoadPolicies(providers, false); err != nil {
		return false, err
	}

	success, fails, err := e.selfTester.RunSelfTest()
	if err != nil {
		return true, err
	}
	...
	// send the report
	if e.config.SelfTestSendReport {
		ReportSelfTest(e.eventSender, e.statsdClient, success, fails)
	}
...
func (e *RuleEngine) LoadPolicies(policyProviders []rules.PolicyProvider, sendLoadedReport bool) error {
	...
	if probeEvaluationRuleSet != nil {
		...
		displayApplyRuleSetReport(report)
...
func (e *RuleEngine) RuleMatch(rule *rules.Rule, event eval.Event) {
	...
	if e.selfTester == nil || !e.selfTester.IsExpectedEvent(rule, event, e.probe) {
...
func displayApplyRuleSetReport(report *kfilters.ApplyRuleSetReport) {

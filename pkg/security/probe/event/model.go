...
func (m *Model) ValidateField(field eval.Field, fieldValue eval.FieldValue) error {
	...
	switch field {
	case "bpf.map.name":
		if offset, found := m.constantOffsets[constantfetch.OffsetNameBPFMapStructName]; !found || offset == constantfetch.ErrorSentinel {
			return fmt.Errorf("%s is not available on this kernel version", field)
		}

	case "bpf.prog.name":
		if offset, found := m.constantOffsets[constantfetch.OffsetNameBPFProgAuxStructName]; !found || offset == constantfetch.ErrorSentinel {
			return fmt.Errorf("%s is not available on this kernel version", field)
		}
	}
...
// NewEvent returns a new event
func NewEvent(ctx *probectx.ProbeCtx) *Event {
	return &Event{
		Event: model.Event{},
		ctx:   ctx,
	}
...
func (ev *Event) ResolveFilePath(f *model.FileEvent) string {
	if !f.IsPathnameStrResolved && len(f.PathnameStr) == 0 {
		path, err := ev.ctx.Resolver.ResolveFileFieldsPath(&f.FileFields, &ev.PIDContext)
		if err != nil {
...
func (ev *Event) ResolveFileBasename(f *model.FileEvent) string {
	if !f.IsBasenameStrResolved && len(f.BasenameStr) == 0 {
		if f.PathnameStr != "" {
			...
			f.SetBasenameStr(ev.ctx.Resolver.ResolveBasename(&f.FileFields))
		}
	}
...
func (ev *Event) ResolveFileFilesystem(f *model.FileEvent) string {
	if f.Filesystem == "" {
		fs, err := ev.ctx.MountResolver.GetFilesystem(f.FileFields.MountID, ev.PIDContext.Pid)
		if err != nil {
			ev.SetPathResolutionError(err)
		}
		f.Filesystem = fs
...
func (ev *Event) SetMountPoint(e *model.MountEvent) error {
	var err error
	e.MountPointStr, err = ev.ctx.DentryResolver.Resolve(e.ParentMountID, e.ParentInode, 0, true)
	return err
}
...
func (ev *Event) SetMountRoot(e *model.MountEvent) error {
	var err error
	e.RootStr, err = ev.ctx.DentryResolver.Resolve(e.RootMountID, e.RootInode, 0, true)
	return err
}
...
func (ev *Event) ResolveContainerTags(e *model.ContainerContext) []string {
	if len(e.Tags) == 0 && e.ID != "" {
		e.Tags = ev.ctx.TagsResolver.Resolve(e.ID)
	}
	return e.Tags
}
...
func (ev *Event) UnmarshalProcessCacheEntry(data []byte) (int, error) {
	entry := ev.ctx.ProcessResolver.NewProcessCacheEntry(ev.PIDContext)
...
func (ev *Event) ResolveFileFieldsUser(e *model.FileFields) string {
	if len(e.User) == 0 {
		e.User, _ = ev.ctx.UserGroupResolver.ResolveUser(int(e.UID))
	}
	return e.User
}
...
func (ev *Event) ResolveFileFieldsGroup(e *model.FileFields) string {
	if len(e.Group) == 0 {
		e.Group, _ = ev.ctx.UserGroupResolver.ResolveGroup(int(e.GID))
	}
	return e.Group
}
...
func (ev *Event) ResolveProcessArgv0(process *model.Process) string {
	arg0, _ := ev.ctx.ProcessResolver.GetProcessArgv0(process)
	return arg0
}
...
func (ev *Event) ResolveProcessArgv(process *model.Process) []string {
	argv, _ := ev.ctx.ProcessResolver.GetProcessArgv(process)
	return argv
}
...
func (ev *Event) ResolveProcessEnvp(process *model.Process) []string {
	envp, _ := ev.ctx.ProcessResolver.GetProcessEnvp(process)
	return envp
}
...
func (ev *Event) ResolveProcessArgsTruncated(process *model.Process) bool {
	_, truncated := ev.ctx.ProcessResolver.GetProcessArgv(process)
	return truncated
}
...
func (ev *Event) ResolveSetuidEUser(e *model.SetuidEvent) string {
	if len(e.EUser) == 0 && ev != nil {
		e.EUser, _ = ev.ctx.UserGroupResolver.ResolveUser(int(e.EUID))
	}
	return e.EUser
}
...
func (ev *Event) ResolveSetuidFSUser(e *model.SetuidEvent) string {
	if len(e.FSUser) == 0 && ev != nil {
		e.FSUser, _ = ev.ctx.UserGroupResolver.ResolveUser(int(e.FSUID))
	}
	return e.FSUser
}
...
func (ev *Event) ResolveSetgidGroup(e *model.SetgidEvent) string {
	if len(e.Group) == 0 && ev != nil {
		e.Group, _ = ev.ctx.UserGroupResolver.ResolveUser(int(e.GID))
	}
	return e.Group
}
...
func (ev *Event) ResolveSetgidEGroup(e *model.SetgidEvent) string {
	if len(e.EGroup) == 0 && ev != nil {
		e.EGroup, _ = ev.ctx.UserGroupResolver.ResolveUser(int(e.EGID))
	}
	return e.EGroup
}
...
func (ev *Event) ResolveSetgidFSGroup(e *model.SetgidEvent) string {
	if len(e.FSGroup) == 0 && ev != nil {
		e.FSGroup, _ = ev.ctx.UserGroupResolver.ResolveUser(int(e.FSGID))
	}
	return e.FSGroup
}
...
func (ev *Event) ResolveSELinuxBoolName(e *model.SELinuxEvent) string {
	...
	if len(ev.SELinux.BoolName) == 0 {
		ev.SELinux.BoolName = ev.ctx.Resolver.ResolveBasename(&e.File.FileFields)
	}
...
func (ev *Event) ResolveEventTimestamp() time.Time {
	if ev.Timestamp.IsZero() {
		ev.Timestamp = ev.ctx.TimeResolver.ResolveMonotonicTimestamp(ev.TimestampRaw)
		if ev.Timestamp.IsZero() {
...
func (ev *Event) ResolveProcessCacheEntry() *model.ProcessCacheEntry {
	if ev.ProcessCacheEntry == nil {
		ev.ProcessCacheEntry = ev.ctx.ProcessResolver.Resolve(ev.PIDContext.Pid, ev.PIDContext.Tid)
	}

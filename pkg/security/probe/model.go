...
func (ev *Event) ResolveMountPointPath(e *model.MountEvent) string {
	if len(e.MountPointPath) == 0 {
		_, mountPointPath, _, err := ev.resolvers.MountResolver.GetMountPath(e.MountID, ev.PIDContext.Pid)
		if err != nil {
...
func (ev *Event) ResolveMountSourcePath(e *model.MountEvent) string {
	if e.BindSrcMountID != 0 && len(e.MountSourcePath) == 0 {
		_, bindSourceMountPath, _, err := ev.resolvers.MountResolver.GetMountPath(e.BindSrcMountID, ev.PIDContext.Pid)
		if err != nil {

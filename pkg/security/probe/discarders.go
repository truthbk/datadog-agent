...
func (id *inodeDiscarders) discardParentInode(req *erpc.ERPCRequest, rs *rules.RuleSet, eventType model.EventType, field eval.Field, filename string, pathKey model.PathKey, timestamp uint64) (bool, uint32, uint64, error) {
	...
	if id.isRecentlyAdded(pathKey.MountID, pathKey.Inode, timestamp) {
		return false, 0, 0, nil
	}

	if err := id.discardInode(req, eventType, pathKey.MountID, pathKey.Inode, false); err != nil {
		return false, 0, 0, err
	}

	id.recentlyAdded(pathKey.MountID, pathKey.Inode, timestamp)

	return true, pathKey.MountID, pathKey.Inode, nil
}
...
func dumpInodeDiscarders(resolver *dentry.Resolver, inodeMap *ebpf.Map) ([]InodeDiscarderDump, error) {
	...
	for entries := inodeMap.Iterate(); entries.Next(&inodeEntry, &inodeParams); {
		...
		path, _, err := resolver.Resolve(inodeEntry.PathKey, false)
		if err == nil {

...
func NewActivityDumpLoadController(adm *ActivityDumpManager) (*ActivityDumpLoadController, error) {
	activityDumpConfigDefaultsMap, found, err := adm.ctx.Manager.GetMap("activity_dump_config_defaults")
	if err != nil {
		return nil, err
	}
	if !found {
...
func (lc *ActivityDumpLoadController) PushCurrentConfig() error {
	...
	defaults := NewActivityDumpLoadConfig(
		lc.adm.ctx.Config.ActivityDumpTracedEventTypes,
		lc.adm.ctx.Config.ActivityDumpCgroupDumpTimeout,
		0,
		lc.adm.ctx.Config.ActivityDumpRateLimiter,
		time.Now(),
		lc.adm.ctx.TimeResolver,
	)
	defaults.WaitListTimestampRaw = uint64(lc.adm.ctx.Config.ActivityDumpCgroupWaitListTimeout)
...
func (lc *ActivityDumpLoadController) sendLoadControllerTriggeredMetric(tags []string) error {
	if err := lc.adm.ctx.StatsdClient.Count(metrics.MetricActivityDumpLoadControllerTriggered, 1, tags, 1.0); err != nil {

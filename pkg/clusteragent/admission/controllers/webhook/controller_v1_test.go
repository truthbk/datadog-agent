...
func TestAdmissionControllerFailureModeIgnore(t *testing.T) {
	...
	webhookSkeleton := c.getWebhookSkeleton("foo", "/bar", []admiv1.OperationType{admiv1.Create}, []string{"pods"}, false)
...
func TestGetWebhookSkeletonV1(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.EqualValues(t, tt.want, c.getWebhookSkeleton(tt.args.nameSuffix, tt.args.path, []admiv1.OperationType{admiv1.Create}, []string{"pods"}, false))
		})
	}
}
...
func TestAdmissionControllerReinvocationPolicyV1(t *testing.T) {
	...
	c.config = NewConfig(true, false)
	webhook := c.getWebhookSkeleton("foo", "/bar", []admiv1.OperationType{admiv1.Create}, []string{"pods"}, false)

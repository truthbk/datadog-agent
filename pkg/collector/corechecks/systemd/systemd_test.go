...
`)
	...
	check.Configure(aggregator.NewNoOpSenderManager(), integration.FakeConfigHash, rawInstanceConfig, []byte(``), "test")
...
`)
	...
	systemdCheck.Configure(aggregator.NewNoOpSenderManager(), integration.FakeConfigHash, rawInstanceConfig, nil, "test")
	// run
	systemdCheck.Run()
...
`)
	...
	err := check1.Configure(aggregator.NewNoOpSenderManager(), integration.FakeConfigHash, rawInstanceConfig1, []byte(``), "test")
	assert.Nil(t, err)

	err = check2.Configure(aggregator.NewNoOpSenderManager(), integration.FakeConfigHash, rawInstanceConfig2, []byte(``), "test")
	assert.Nil(t, err)

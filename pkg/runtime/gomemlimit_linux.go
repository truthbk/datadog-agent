...
func NewStaticMemoryLimiter(limitPct float64, isContainerized bool) MemoryLimiter {
...
func (d dynamicMemoryLimiter) computeSetLimit() {
	...
	if err := d.selfCgroup.GetMemoryStats(&cgroupMemStats); err != nil {
		log.Warnf("Unable to get self memory stats, dynamic memory limiter unavailable")
		return
	}

...
func SetupLogAgent(logChannel chan *config.ChannelMessage, sourceName string, source string, logSyncOrchestrator *logsyncorchestrator.LogSyncOrchestrator) {
	agent, err := logs.StartServerless(logSyncOrchestrator)
	if err != nil {

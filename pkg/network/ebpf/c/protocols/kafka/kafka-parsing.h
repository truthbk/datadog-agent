...
static __always_inline bool kafka_process(kafka_transaction_t *kafka_transaction, struct __sk_buff* skb, skb_info_t *skb_info);
...
//        log_debug("socket__kafka_filter failed to fetch arguments for tail call\n");
    ...
    (void)kafka_process(kafka, skb, &skb_info);
    return 0;
}
...
static __always_inline bool kafka_process(kafka_transaction_t *kafka_transaction, struct __sk_buff* skb, skb_info_t *skb_info) {
    ...
    */
    ...
    bpf_skb_load_bytes(skb, skb_info->data_off, (char *)&kafka_header, sizeof(kafka_header));
...
//    log_debug("kafka: kafka_header.api_version: %d\n", kafka_header.api_version);
    ...
    read_into_buffer_topic_name_parser((char *)kafka_transaction->base.topic_name, skb, skb_info->data_off, skb_info->data_end);

...
#define READ_INTO_BUFFER(name, total_size, blk_size)                                                                        \
    static __always_inline void read_into_buffer_##name(char *buffer, struct __sk_buff *skb, u32 offset, u32 data_end) {    \
        const u32 end = (total_size) < (data_end - offset) ? offset + (total_size) : data_end;                              \
        unsigned i = 0;                                                                                                     \
                                                                                                                            \

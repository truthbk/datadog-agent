...
#define READ_BIG_ENDIAN(type, transformer)                                                                          \
    static __always_inline bool read_big_endian_##type(struct __sk_buff *skb, skb_info_t *skb_info, type *out) {    \
        if (skb_info->data_off + sizeof(type) > skb_info->data_end) {                                               \
            return false;                                                                                           \
        }                                                                                                           \
        type val;                                                                                                   \
        bpf_memset(&val, 0, sizeof(type));                                                                          \
        bpf_skb_load_bytes_with_telemetry(skb, skb_info->data_off, &val, sizeof(type));                             \
        *out = transformer(val);                                                                                    \
        return true;                                                                                                \
    }

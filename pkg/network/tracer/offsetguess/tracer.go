...
func (t *tracerOffsetGuesser) checkAndUpdateCurrentOffset(mp *ddebpf.GenericMap[uint64, TracerStatus], expected *fieldValues, maxRetries *int, threshold uint64) error {
...
func (t *tracerOffsetGuesser) setReadyState(mp *ddebpf.GenericMap[uint64, TracerStatus]) error {
...
func (t *tracerOffsetGuesser) Guess(cfg *config.Config) ([]manager.ConstantEditor, error) {
	mp, err := ddebpf.GetMap[uint64, TracerStatus](t.m, probes.TracerStatusMap)
	if err != nil {

...
func newBatchManager(batchMap, batchStateMap *ebpf.Map, numCPUs int) (*batchManager, error) {
	...
	for i := 0; i < numCPUs; i++ {
		...
		err := batchStateMap.Put(unsafe.Pointer(&i), unsafe.Pointer(state))
		if err != nil {
			return nil, fmt.Errorf("can't initialize batchStateMap : %w", err)
		}
		for j := 0; j < HTTPBatchPages; j++ {
			...
			if err != nil {
				return nil, fmt.Errorf("can't initialize batchMap : %w", err)
			}
		}
	}

...
func (p *GoTLSProgram) handleProcessStart(pid pid) {
	...
	// Check go process
	probeList := make([]manager.ProbeIdentificationPair, 0)
	p.registry.Register(binPath, pid, registerCBCreator(p.manager, p.offsetsDataMap, &probeList, p.binAnalysisMetric), detachHooks(p.manager, &probeList, p.offsetsDataMap))
}
...
func detachHooks(mgr *errtelemetry.Manager, probeIDs *[]manager.ProbeIdentificationPair, offsetsDataMap *ebpf.Map) func(path utils.FilePath) error {
	return func(path utils.FilePath) error {
		...
		for _, probeID := range *probeIDs {
			log.Debugf("detaching %s/%s from %s", probeID.EBPFFuncName, probeID.UID, path.HostPath)

...
func TestTransactionRetryQueueAdd(t *testing.T) {
	...
	container := NewTransactionRetryQueue(createDropPrioritySorter(), q, 100, 0.6, NewTransactionRetryQueueTelemetry("domain"), NewTelemetrySenderMock())
...
func TestTransactionRetryQueueSeveralFlushToDisk(t *testing.T) {
	...
	container := NewTransactionRetryQueue(createDropPrioritySorter(), q, 50, 0.1, NewTransactionRetryQueueTelemetry("domain"), NewTelemetrySenderMock())
...
func TestTransactionRetryQueueNoTransactionStorage(t *testing.T) {
	a := assert.New(t)
	pointDropped := transactionContainerPointDroppedCountTelemetry.expvar.Value()
	container := NewTransactionRetryQueue(createDropPrioritySorter(), nil, 50, 0.1, NewTransactionRetryQueueTelemetry("domain"), NewTelemetrySenderMock())
...
func TestTransactionRetryQueueZeroMaxMemSizeInBytes(t *testing.T) {
	...
	maxMemSizeInBytes := 0
	pointDropped := transactionContainerPointDroppedCountTelemetry.expvar.Value()
	container := NewTransactionRetryQueue(createDropPrioritySorter(), q, maxMemSizeInBytes, 0.1, NewTransactionRetryQueueTelemetry("domain"), NewTelemetrySenderMock())
...
func newOnDiskRetryQueueTest(t *testing.T, a *assert.Assertions) *onDiskRetryQueue {
	...
	q, err := newOnDiskRetryQueue(
		...
		path,
		diskUsageLimit,
		newOnDiskRetryQueueTelemetry("domain"), NewTelemetrySenderMock())
	a.NoError(err)
	return q
}

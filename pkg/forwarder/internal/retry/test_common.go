...
func NewTelemetrySenderMock() *TelemetrySender {
	provider := telemetry.NewStatsTelemetryProvider(StatsTelemetrySenderMock{})
	return NewTelemetrySender("domain", provider)
}
...
func (m StatsTelemetrySenderMock) CountHidden(metric string, value float64, hostname string, tags []string) {
}

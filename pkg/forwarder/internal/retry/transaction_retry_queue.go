...
// TransactionDiskStorage is an interface to storage and load transactions from disk
...
func BuildTransactionRetryQueue(
	...
	telemetrySender := NewTelemetrySender(domain, telemetry.GetStatsTelemetryProvider())

	if optionalDomainFolderPath != "" && optionalDiskUsageLimit != nil {
		serializer := NewHTTPTransactionsSerializer(resolver)
		storage, err = newOnDiskRetryQueue(serializer, optionalDomainFolderPath, optionalDiskUsageLimit, newOnDiskRetryQueueTelemetry(resolver.GetBaseDomain()), telemetrySender)
...
func NewTransactionRetryQueue(
	...
	flushToStorageRatio float64,
	telemetry TransactionRetryQueueTelemetry,
	telemetrySender *TelemetrySender) *TransactionRetryQueue {
...
func (tc *TransactionRetryQueue) onDropPoints(count int) {
	...
	tc.telemetrySender.AddDroppedPointCount(count)
}

...
func (f *SyncForwarder) SubmitV1Series(payload transaction.BytesPayloads, extra http.Header) error {
	transactions := f.defaultForwarder.createHTTPTransactionsPayload(endpoints.V1SeriesEndpoint, payload, true, extra)
...
func (f *SyncForwarder) SubmitSeries(payload transaction.BytesPayloads, extra http.Header) error {
	transactions := f.defaultForwarder.createHTTPTransactionsPayload(endpoints.SeriesEndpoint, payload, true, extra)
	return f.sendHTTPTransactions(transactions)
}
...
func (f *SyncForwarder) SubmitSketchSeries(payload transaction.BytesPayloads, extra http.Header) error {
	transactions := f.defaultForwarder.createHTTPTransactionsPayload(endpoints.SketchSeriesEndpoint, payload, true, extra)
	return f.sendHTTPTransactions(transactions)
}

...
func NodeAgentOptions() (Options, error) {
	...
	if err != nil {
		return Options{}, fmt.Errorf("unable to fetch authentication token: %w", err)
	}
...
func CLCRunnerOptions() (Options, error) {
	...
	token, err := security.GetClusterAgentAuthToken()
	if err != nil {
		return Options{}, fmt.Errorf("unable to fetch authentication token: %w", err)
	}
	...
	return backoff.Retry(func() error {
		...
		t.streamCtx, t.streamCancel = context.WithCancel(
			metadata.NewOutgoingContext(t.ctx, metadata.MD{
				"authorization": []string{fmt.Sprintf("Bearer %s", t.options.Token)},
			}),
		)

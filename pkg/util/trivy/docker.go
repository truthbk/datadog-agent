...
func convertDockerImage(ctx context.Context, client client.ImageAPIClient, imgMeta *workloadmeta.ContainerImageMetadata) (types.Image, error) {
	...
	if err != nil {
		...
		inspect, _, err = client.ImageInspectWithRaw(ctx, imageID)
		if err != nil {
			return nil, xerrors.Errorf("unable to inspect the image (%s): %w", imageID, err)
		}
	}

	history, err := client.ImageHistory(ctx, imageID)
	if err != nil {
		return nil, xerrors.Errorf("unable to get history (%s): %w", imageID, err)
	}

	return &image{
		opener:  imageOpener(ctx, imageID, client.ImageSave),
		inspect: inspect,

...
func readProfileData(cliParams *cliParams, pdata *pkgflare.ProfileDataDCA, seconds int, collector profileCollector) error {
	...
	utillog.Debugf("Hello: reading dca profile data")
...
func serviceProfileCollector(service string, portConfig string, seconds int) agentProfileCollector {
	return func(cliParams *cliParams, pdata *pkgflare.ProfileDataDCA, collector profileCollector) error {
		...
		utillog.Debugf("Hello: pprofurl: %s", pprofURL)
...
func run(log log.Component, config config.Component, cliParams *cliParams) error {
	...
	c := util.GetClient(true) // FIX: get certificates right then make this true
	...
	utillog.Debugf("Hello: e value after DoPost: %s", e)
	var filePath string
	if e != nil {

		utillog.Debugf("Hello: From run when e is different than nil, e: %s", e)
		if r != nil && string(r) != "" {
		...
		utillog.Debugf("Hello: Creating DCAARchive from run")
		filePath, e = flare.CreateDCAArchive(true, common.GetDistPath(), logFile, profile)
		if e != nil {
			fmt.Printf("The flare zipfile failed to be created: %s\n", e)
			return e
		}
	} else {
		utillog.Debugf("Hello: From run when e is nil, e: %s", e)
		filePath = string(r)
	}

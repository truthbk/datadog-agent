...
func start(log log.Component, config config.Component, telemetry telemetry.Component, forwarder defaultforwarder.Component, cliParams *command.GlobalParams) error {
	...
	if pkgconfig.Datadog.GetBool("admission_controller.enabled") {
		...
		if err != nil {
			...
			server.Register(pkgconfig.Datadog.GetString("admission_controller.cws_instrumentation.pod_endpoint"), mutate.InjectCWSPodInstrumentation, apiCl.DynamicCl, apiCl.Cl)
			server.Register(pkgconfig.Datadog.GetString("admission_controller.cws_instrumentation.command_endpoint"), mutate.InjectCWSCommandInstrumentation, apiCl.DynamicCl, apiCl.Cl)

...
```
The developer needs to provide a name to associate with the `stack`, or it will be automatically generated the name from the current branch.
...
- The stack to target specified with [--stack=<name>]. If not specified, a name from the current branch will be generated.

...
func TestGetInstallMethod(t *testing.T) {
	conf := config.Mock(t)
	defer func(orig func(conf config.ConfigReader) (*installinfo.InstallInfo, error)) {
		installinfoGet = orig
	}(installinfoGet)

	installinfoGet = func(config.ConfigReader) (*installinfo.InstallInfo, error) { return nil, fmt.Errorf("an error") }
	...
	installinfoGet = func(config.ConfigReader) (*installinfo.InstallInfo, error) {
